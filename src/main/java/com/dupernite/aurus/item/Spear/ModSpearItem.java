...
public class ModSpearItem extends Item implements Vanishable {
    ...
    public ModSpearItem(Item.Settings settings) {
        super(settings);
        ...
        builder.put(EntityAttributes.GENERIC_ATTACK_SPEED, new EntityAttributeModifier(ATTACK_SPEED_MODIFIER_ID, "Tool modifier", -2.9000000953674316, Operation.ADDITION));
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerEntity.isTouchingWaterOrRain()) {
                    if (!world.isClient) {
                        ...
                        if (j == 0) {
                            ModSpearEntity spearEntity = new ModSpearEntity(world, playerEntity, stack);
                            spearEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, 2.5F + (float)j * 0.5F, 1.0F);
                            if (playerEntity.getAbilities().creativeMode) {
                                spearEntity.pickupType = PickupPermission.CREATIVE_ONLY;
                            }
